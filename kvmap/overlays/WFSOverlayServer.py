...
class WFSOverlayServer(OverlayServer):
	cache = {} 
	...
	def draw_in(self, aMapViewer):
		geometries = None
		if aMapViewer.lastmove is None or time.time() > aMapViewer.lastmove + 0.5: # wait a second after moving before we try to contact the WFS
			geometries = self.get(self, aMapViewer.parent.width, aMapViewer.parent.height)
		if geometries is not None:
			with aMapViewer.canvas:
				for geom in geometries:
					if geom.tag == "{%s}Point" % GMLNS:
						copos = map(float,geom.getchildren()[0].text.split())
						l, m = self.co_to_ll(copos[0], copos[1])
						x,y = aMapViewer.get_xy_from_latlon(l, m) 
						
						Color(0, 0, 0)
						r = 10.0/aMapViewer.scale
						Ellipse(pos=(x-r/2, y-r/2), size=(r,r))
						Color(1, 1, 1)
						r = 8.0/aMapViewer.scale
						Ellipse(pos=(x-r/2, y-r/2), size=(r,r))
						print x,y,r
